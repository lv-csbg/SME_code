function [x, population, scores, optGeneSol, hashtable] = optGene1(model, targetRxn, substrateRxn, generxnList, varargin)
...
parser.addParamValue('population', [], @(x)  ismatrix(x) && ~isvector(x));
...
FitnessFunction = @(x) optGeneFitnessTilt1(x,model,targetRxn, generxnList, geneok);

function [val] = optGeneFitnessTilt1(rxn_vector_matrix, model, targetRxn, rxnListInput, isGeneList)
...
for i = 1:popsize
    ...
    if ~isempty(CBT_LP_SOLVER) && strcmp(CBT_LP_SOLVER,'cplex')
        ...
        if isempty(solKO.x)
            ...
            biomass_reaction = modelKO.c;
            modelKO.c = objFuncGA;
            modelKO.ub(biomass_reaction == 1) = growthrate;
            modelKO.lb(biomass_reaction == 1) = growthrate;
            objFuncResult = optimizeCbModel(modelKO,'min');
            minProdAtSetGR = objFuncResult.f;
            
            modelKO.ub(biomass_reaction == 1) = 0;
            modelKO.lb(biomass_reaction == 1) = 0;
            objFuncZeroGR = optimizeCbModel(modelKO,'min');
            minObjAtZeroGR = objFuncZeroGR.f;
%             modelKO.ub(biomass_reaction == 1) = 0;
%             modelKO.lb(biomass_reaction == 1) = 0;
%             objAtZero = optimizeCbModel(modelKO,'min');
%             objAtZero = objAtZero.f;
        end
    end
    ...
    %sum(rxnList)
    val(i) = -30 - minProdAtSetGR -0.5*minObjAtZeroGR + 1*length(rxnList); 
